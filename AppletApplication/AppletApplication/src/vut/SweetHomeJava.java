...
public class SweetHomeJava extends Applet {
    //@Override
    public void init() {
        setSize(1900, 950);
    ...
    public void paint(Graphics g) {
        ...
        // Set a background color resembling the sky (light cyan or light blue)
        g2.setColor(new Color(135, 206, 235));  // Sky blue color (light cyan)
        g2.fillRect(0, 0, getWidth(), getHeight()); // This fills the background

        // Draw the house and the car
        drawSweetHome(g2);
        myRide(g2);
    ...
    private void displayTextAndLine(Graphics2D g2) {
        ...
        g2.drawString("CREATED BY: 223373052 Mashele Nkgere Louis", 50, 60);
    ...
    // Method to draw the sweet home (house)
    ...
    // Method to draw the car (myRide)
    private void myRide(Graphics2D g2) {
        g2.setColor(Color.RED);  // Body color of the car
        g2.fillRoundRect(600, 750, 500, 100, 20, 20); // Car body
        
        // Windows of the car
        g2.setColor(Color.WHITE);
        g2.fillRoundRect(630, 760, 100, 50, 10, 10); // Front window
        g2.fillRoundRect(750, 760, 100, 50, 10, 10); // Back window
        g2.fillRoundRect(870, 760, 100, 50, 10, 10); // Rear window
        
        // Car front lights (left and right)
        g2.setColor(Color.YELLOW);
        g2.fillOval(610, 780, 20, 20);  // Left light
        g2.fillOval(1070, 780, 20, 20);  // Right light

        // Draw the wheels with black tires and 6 spokes
        drawWheel(g2, 650, 820);  // Front wheel
        drawWheel(g2, 1050, 820); // Rear wheel

        // Draw the steering wheel and driver's seat
        g2.setColor(Color.BLACK);
        g2.fillOval(880, 770, 20, 20); // Steering wheel
        g2.setColor(Color.GRAY);
        g2.fillRect(920, 780, 50, 20); // Driver's seat
    }

    // Method to draw the wheels with 6 spokes
    private void drawWheel(Graphics2D g2, int x, int y) {
        // Outer tire (black and thick)
        g2.setColor(Color.BLACK);
        g2.fillOval(x - 40, y - 40, 80, 80); // Tire

        // Inner wheel (gray)
        g2.setColor(Color.DARK_GRAY);
        g2.fillOval(x - 30, y - 30, 60, 60); // Inner wheel

        // Drawing 6 spokes for each wheel
        g2.setColor(Color.WHITE);
        for (int i = 0; i < 6; i++) {
            double angle = Math.toRadians(i * 60);  // 60 degrees for each spoke
            int startX = (int) (x + 30 * Math.cos(angle)); // Start X for spoke
            int startY = (int) (y + 30 * Math.sin(angle)); // Start Y for spoke
            int endX = (int) (x + 40 * Math.cos(angle)); // End X for spoke
            int endY = (int) (y + 40 * Math.sin(angle)); // End Y for spoke
            g2.drawLine(startX, startY, endX, endY); // Draw spoke
        }
    }
